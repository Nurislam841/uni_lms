    ...
    new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try { resolve(xhr.responseText ? JSON.parse(xhr.responseText) : ({} as T)); } catch { resolve({} as T); }
        } else {
          try { reject(new ApiError(xhr.status, JSON.parse(xhr.responseText)?.message ?? `Error ${xhr.status}`)); } catch { reject(new ApiError(xhr.status, `Error ${xhr.status}`)); }
        }
      };
